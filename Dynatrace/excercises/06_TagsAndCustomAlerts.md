...
Open your "Training-<Month>-0XX" environment on the [training-cluster](https://dev.eu10.apm.services.cloud.sap/login) and verify that your process group contains the properties `Team Operations` and `Stack CF`.
...
2) "Add a new rule"
3) Add `{ProcessGroup:Environment:Team}` as tag value.
4) Select "Process groups" in the Drop down "Rule applies to".
5) Add a condition: 
...
7) Click on "Preview", you should see your spring music process group, host and services being listed.
8) Click on "Create rule".
9) Save the tagging rule by clicking "Save changes" in the bottom of the screen.
...
2) Open your "Training-<Month>-0XX" environment on the [training-cluster](https://dev.eu10.apm.services.cloud.sap/login).
3) Select the "Manage" -> "Settings" tab in the Dynatrace UI.
4) Click on "Anomaly detection" and choose "Metric events".
5) Click "Add metric event".
6) In the "Summary" field, Enter any title e.g. "4XX threshold breached".
7) In the **Query definition** section choose "Metric key" as Type and "Number of HTTP 4xx errors" as the metric key. choose "Value" as aggregation type.
8) Configure the following "Static threshold" in the **Monitoring strategy** section:
   "Alert anomalies with a static threshold of **1** **Count (count)**"
   At "Advanced model properties": "Raise an alert if the metric is **above** the threshold for **1** minutes during any **3** minute period."
   Make sure the "Dealerting samples" is <= Sliding window
9) Under "Event template"
- Enter any title e.g. "4XX threshold breached".
- Select "Custom alert" as Event type.
10) Save the changes by clicking "Save changes" at the bottom of the screen.
