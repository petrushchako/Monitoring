...
In this exercise we will use the Multi-Dimensional Analysis view to configure the calculated service metric (the alternative configuration possibility would be via "Manage" -> "Settings" -> "Server-side service monitoring" -> "Calculated service metrics").
   ...
   1. Go to "Application Observability" -> "Multidimensional analysis" -> "Top web requests"
...
3. Select "Single value" in the visualizations drop down just above the metric drop down box and run the query. This will show you now the sum of request counts over all services with response code 418 in the selected time frame.
...
1. Go to "Manage" -> "Settings" -> "Anomaly detection" -> "Metric events"
2. Click on "Add metric event".
3. In the "Summary" field, Enter any title e.g. "Response code 418 detected".
4. In the **Query definition** section choose "Metric key" as Type and "Request Count with Response Code 418" as the metric key. choose "Value" as aggregation type.
5. Configure the following "Static threshold" in the **Monitoring strategy** section:
