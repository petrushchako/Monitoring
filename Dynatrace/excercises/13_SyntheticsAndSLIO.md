...
1) Click on "Application Observability" -> "Synthetic" tab on the right hand side.
...
4) Click on the dropdown and choose the metric "builtin:synthetic.http.availability.location.total" in the metric expression box.  ***This metric is only created after the Synthetic Test runs, so please be patient.
5) Select "Last 1 week" for the timeframe and delete any predefined entity selector filter if there is any under "Define a filter".
6) Define the SLO target (Failure - 99.95 , Warning - 99.97) under "Add success criteria"
7) Check your settings and click on "Create".
